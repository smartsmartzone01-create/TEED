...
class TradeInInputSerializer(serializers.Serializer):
    ...
    # Kept for backwards compatibility with drafts created by the first trade-in UI.
    # Stock organization is now generated by TEED and this value is not user-facing.
...
class SaleSerializer(serializers.ModelSerializer):
    ...
    def get_gross_profit(self, obj):
        if obj.transaction_type == Sale.TransactionType.TRADE_IN:
            try:
                detail = obj.trade_in_detail
            except TradeInDetail.DoesNotExist:
                return obj.total - obj.cost_of_goods
            consideration = detail.incoming_value + detail.cash_top_up
            return consideration - obj.cost_of_goods
        return obj.total - obj.cost_of_goods
