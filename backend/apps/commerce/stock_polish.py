...
def _require_whole_quantity(*, quantity, unit, field):
    ...
    if _is_countable(unit) and quantity != quantity.to_integral_value():
        raise ValidationError(
            {
                field: [
                    f"{unit.title()} is a countable unit. Enter a whole number, "
...
def _assert_receipt_editable(receipt):
    ...
    if (
        ...
        and timezone.now() > _correction_deadline(receipt)
    ):
        raise ValidationError(
            {
                "receipt": [
                    "The 48-hour stock correction window has closed. Use Add late "
                    "delivery for goods that arrived later, or record a governed "
                    "inventory correction instead of rewriting this receipt."
                ]
            }
        )
...
class PolishedStockReceiptSerializer(StockReceiptSerializer):
    ...
    class Meta(StockReceiptSerializer.Meta):
        fields = [
            *StockReceiptSerializer.Meta.fields,
            "correction_open",
            "correction_deadline",
        ]
    ...
    def validate(self, attrs):
        ...
        existing_units = dict(
            Product.objects.filter(
                id__in=product_ids, business_id=business_id
            ).values_list("id", "unit")
        )
        for item in attrs.get("catalog_items", []):
        ...
        for batch_index, batch in enumerate(attrs["batches"], start=1):
            for group_index, group in enumerate(batch.get("groups", []), start=1):
                if (
                    _is_countable(group["unit"])
                    and group["quantity"] != group["quantity"].to_integral_value()
                ):
                    raise ValidationError(
                        {
                            "batches": [
                ...
                for type_index, line in enumerate(group.get("types", []), start=1):
                    ...
                    if (
                        _is_countable(unit)
                        and line["quantity_received"]
                        != line["quantity_received"].to_integral_value()
                    ):
                        raise ValidationError(
                            {
                                "batches": [
                                    f"Batch {batch_index}, group {group_index}, "
                                    f"product {type_index}: {unit} requires a whole "
                                    "quantity."
                                ]
                            }
                        )
            for line_index, line in enumerate(batch.get("products", []), start=1):
                unit = line.get("received_unit") or catalog_units.get(
                    line.get("catalog_key"), ""
                )
                ...
                if (
                    ...
                    raise ValidationError(
                        {
                            "batches": [
                                f"Batch {batch_index}, product {line_index}: {unit} "
                                "requires a whole quantity."
                            ]
                        }
                    )
        return attrs
        ...
        if new_unit.casefold() != old_unit.casefold():
            if line.stock_group_id:
                raise ValidationError(
                    {
                        "lines": [
                            f"{product.name}: grouped stock units cannot be rewritten "
                            "from one item line."
                        ]
                    }
                )
            if (
                ...
                raise ValidationError(
                    {
                        "lines": [
                            f"{product.name}: the unit cannot be changed because this "
                            "stock already has dependent activity."
                        ]
                    }
                )
            if product.stock_batches.exclude(pk=line.pk).exists():
                raise ValidationError(
                    {
                        "lines": [
                            f"{product.name}: the unit is already used by other stock "
                            "receipts."
                        ]
                    }
                )
            if line.conversion_to_base != Decimal("1"):
                raise ValidationError(
                    {
                        "lines": [
                            f"{product.name}: converted units require a governed "
        ...
        if quantity is not None:
            ...
            if line.tracking_mode == Product.TrackingMode.INDIVIDUAL:
                if new_received != new_received.to_integral_value():
                    raise ValidationError(
                        {
                            "lines": [
                                f"{product.name}: individual items require a whole "
                                "quantity."
                            ]
                        }
                    )
                ...
                if tracked_count != int(new_received):
                    raise ValidationError(
                        {
                            "lines": [
                                f"{product.name}: quantity must match the "
                                f"{tracked_count} individual item records already saved."
                            ]
                        }
                    )
            if new_received < consumed:
                raise ValidationError(
                    {
                        "lines": [
                            f"{product.name}: quantity cannot be reduced below "
                            f"{consumed}, because that amount has already left "
                            "available stock."
                        ]
                    }
                )
            if line.stock_group_id:
                if line.stock_group.type_lines.exclude(pk=line.pk).exists():
                    raise ValidationError(
                        {
                            "lines": [
                                f"{product.name}: correct the complete group rather "
            ...
            if receipt.status == StockReceipt.Status.RECEIVED:
                ...
                if line.quantity_remaining < 0:
                    raise ValidationError(
                        {
                            "lines": [
                                f"{product.name}: this correction would create negative "
                                "available stock."
                            ]
                        }
                    )
                if delta:
                    ...
                    InventoryMovement.objects.create(
                        ...
                        reason=(
                            f"Human-error correction to {receipt.reference} within the "
...
class StockReceiptListCreatePolishAPIView(CommerceBaseAPIView):
    ...
    def post(self, request, business_id):
        ...
        return SuccessResponse(
            message="Stock draft saved successfully."
