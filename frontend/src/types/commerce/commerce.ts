type Product = {
  id: string; name: string; sku: string; variant: string; unit: string;
  selling_price: string; low_stock_threshold: string; current_quantity: string;
  is_active: boolean;
};
...
type CommercePulse = {
  ...
  low_stock_count: number; stock_value: string | null;
  confidence: "partial" | "reliable"; can_manage_finance: boolean;
};

type CommerceOverview = { pulse: CommercePulse; decisions: Decision[]; recent_sales: Sale[] };

export type { CommerceOverview, CommercePulse, Decision, Product, Sale, SaleItem };
