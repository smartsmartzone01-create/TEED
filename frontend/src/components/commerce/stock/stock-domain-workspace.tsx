...
const countableUnits = new Set(unitOptions.slice(0, 15));
...
const cleanQuantity = (value: string | number) =>
  new Intl.NumberFormat(undefined, { maximumFractionDigits: 3 }).format(
    Number(value),
  );
const unitCost = (mode: StockCostMode, value: string, quantity: string) => {
...
function CostEditor({
  ...
  const normalized = unitCost(mode, value, quantity);
    ...
    <label className={field}>
      ...
      {value && normalized != null ? (
        <span className="font-normal text-slate-500">
          {mode === "total"
            ? `${t("fields.costPerUnit")}: ${money(normalized)}`
...
function StockDomainWorkspace({ businessId }: { businessId: string }) {
  ...
  const makeTrackedUnit = (): DraftTrackedUnit => ({
    id: nextId("unit"),
    modelName: "",
    brand: "",
    color: "",
  ...
  const makeProduct = (): DraftProduct => ({
    id: nextId("product"),
    existingProductId: "",
    name: "",
    brand: "",
  ...
  const makeBatch = (name: string): DraftBatch => ({
    id: nextId("batch"),
    name,
    groups: [makeGroup()],
  });
  ...
  const updateBatch = (id: string, change: (value: DraftBatch) => DraftBatch) =>
    setBatches((all) => all.map((item) => (item.id === id ? change(item) : item)));
  const updateGroup = (
    batchId: string,
    id: string,
    change: (value: DraftGroup) => DraftGroup,
  ) =>
    updateBatch(batchId, (batch) => ({
      ...batch,
      groups: batch.groups.map((item) => (item.id === id ? change(item) : item)),
    }));
  const updateProduct = (
    batchId: string,
    groupId: string,
    id: string,
    change: (value: DraftProduct) => DraftProduct,
  ) =>
    updateGroup(batchId, groupId, (group) => ({
      ...group,
      products: group.products.map((item) => (item.id === id ? change(item) : item)),
    }));
  ...
  const chooseProduct = (
    ...
    if (product && product.unit.toLowerCase() !== groupUnit.toLowerCase()) {
    ...
    updateProduct(batchId, group.id, draft.id, (current) => ({
      ...
      trackingMode: product?.tracking_mode ?? "quantity",
      trackedUnits: [],
    }));
  };
  ...
  const validateDraft = () => {
    for (const batch of batches) {
      ...
      for (const group of batch.groups) {
        ...
        if (countableUnits.has(unit as (typeof unitOptions)[number]) && !Number.isInteger(quantity))
          return t("validation.wholeQuantity", { unit });
        const allocated = group.products.reduce(
          (total, product) => total + Number(product.quantity || 0),
          0,
        );
        if (allocated !== quantity) return stockT("validation.groupBalance");
        for (const product of group.products) {
          ...
          if (product.costValue && unitCost(product.costMode, product.costValue, product.quantity) == null)
            return stockT("validation.buyingCost");
          if (product.trackingMode === "individual") {
            if (!Number.isInteger(productQuantity)) return stockT("validation.individualWhole");
            if (product.trackedUnits.length !== productQuantity)
              return stockT("validation.individualCount", { count: productQuantity });
          }
        }
      }
    }
        ...
        groups: batch.groups.map((group) => {
          ...
          return {
            ...
            types: group.products.map((product) => ({
              ...
              ...(product.costValue
                ? { unit_cost: unitCost(product.costMode, product.costValue, product.quantity) }
                : {}),
  ...
  const resetDraft = (parent?: StockReceipt) => {
  ...
  const updateCorrectionGroup = (
    batchId: string,
    groupId: string,
    change: (group: CorrectionGroup) => CorrectionGroup,
  ) => {
  ...
  const updateCorrectionProduct = (
    ...
    groupId: string,
    productId: string,
    change: (product: CorrectionProduct) => CorrectionProduct,
  ) =>
    ...
    try {
      await correctStockReceipt(businessId, receipt.id, accessToken, {
        ...
        groups: correction.batches.flatMap((batch) =>
          batch.groups.map((group) => ({ id: group.id, name: group.name.trim() })),
        ),
          ...
          batch.groups.flatMap((group) =>
            group.products.map((product) => ({
              ...
              unit_cost: product.costValue
                ? unitCost(product.costMode, product.costValue, product.quantity)
                : null,
            })),
          ),
        ),
      ...
      <section className={`${panel} grid gap-5`}>
            ...
            {batch.groups.map((group) => (
              <section className="grid gap-3 rounded-xl bg-slate-50 p-3 dark:bg-slate-900" key={group.id}>
                <div className="flex flex-wrap justify-between gap-2">
                  ...
                  <span className="text-sm">{cleanQuantity(group.quantity)} {group.unit}</span>
                </div>
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-900">
            ...
            <p className="font-semibold">{money(savedReceipt.total_buying_value)}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-900">
            <span className="text-slate-500">{stockT("fields.stockExpenses")}</span>
            <p className="font-semibold">{money(savedReceipt.additional_cost)}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-900">
            <span className="text-slate-500">{stockT("summary.totalStockCost")}</span>
            <p className="font-semibold">
              {money(Number(savedReceipt.total_buying_value) + Number(savedReceipt.additional_cost))}
            </p>
          </div>
        </div>
        <StockSummaryActions receipt={savedReceipt} />
        <Button type="button" onClick={() => resetDraft()}>{t("actions.recordAnotherStock")}</Button>
      </section>
    );
  }
    ...
    <section className="grid items-start gap-5 xl:grid-cols-[1.2fr_.8fr]">
      <form className={`${panel} grid gap-5`} onSubmit={submitStock}>
        ...
        {batches.map((batch, batchIndex) => (
          <article className="grid gap-4 rounded-xl border border-slate-200 p-4 shadow-sm dark:border-slate-800" key={batch.id}>
            ...
            {batch.groups.map((group) => {
              const activeUnit = group.unit === "other" ? group.customUnit : group.unit;
              const allocated = group.products.reduce(
                (total, product) => total + Number(product.quantity || 0),
                0,
              );
              return (
                <section className="grid gap-4 rounded-xl bg-slate-50 p-4 dark:bg-slate-900" key={group.id}>
                  ...
                  <p className="text-xs text-slate-500">
                    {stockT("groupBalance", { recorded: allocated, total: Number(group.quantity || 0), unit: activeUnit || "—" })}
                  </p>
                    ...
                    <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950" key={product.id}>
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        {!product.existingProductId ? (
                          ...
                          <div className="self-end rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-800"><strong>{product.name}</strong></div>
                        )}
                        {!product.existingProductId ? (
                          <>
                            <label className={field}>{t("fields.brandOptional")}<Input value={product.brand} onChange={(event) => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, brand: event.target.value }))} /></label>
                            <label className={field}>{t("fields.variant")}<Input value={product.variant} onChange={(event) => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, variant: event.target.value }))} /></label>
                          </>
                        ) : null}
                        <label className={field}>
                        ...
                        <CostEditor
                          ...
                          onMode={(mode) => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, costMode: mode }))}
                          onValue={(value) => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, costValue: value }))}
                        />
                        <label className={field}>
                          {t("fields.tracking")}
                          ...
                          {product.trackedUnits.map((tracked) => (
                            <div className="grid gap-2 rounded-lg bg-slate-50 p-3 sm:grid-cols-2 dark:bg-slate-900" key={tracked.id}>
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button type="button" variant="ghost" onClick={() => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, trackedUnits: current.trackedUnits.filter((item) => item.id !== tracked.id) }))}><Trash2 className="size-4" /></Button>
                              </div>
                            </div>
                          ))}
                          <Button disabled={product.trackedUnits.length >= Math.floor(Number(product.quantity) || 0)} type="button" variant="outline" onClick={() => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, trackedUnits: [...current.trackedUnits, makeTrackedUnit()] }))}>
      ...
      <aside className={`${panel} self-start`}>
        ...
        <div className="mt-4 grid gap-3 xl:max-h-[calc(100vh-12rem)] xl:overflow-y-auto xl:pr-2">
          {(showAll ? receipts : receipts.slice(0, 5)).map((receipt, index) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div><strong>{receipt.reference}</strong><p className="text-xs text-slate-500">{receipt.supplier_name || t("fields.supplierNotEntered")}</p></div>
                ...
                <div className="grid gap-2 border-t border-slate-100 pt-3 dark:border-slate-800" key={batch.id}>
                  ...
                  {batch.groups.map((group) => (
                    <div className="grid gap-2 rounded-lg bg-slate-50 p-2 dark:bg-slate-900" key={group.id}>
                      <div className="flex justify-between gap-2"><strong>{group.name}</strong><span>{cleanQuantity(group.quantity)} {group.unit}</span></div>
                      {group.types.map((line) => (
                        <div className="flex justify-between gap-3 text-xs" key={line.id}>
                          ...
                          <span>{money(line.received_unit_cost)} / {line.received_unit}</span>
                        </div>
                      ))}
                    </div>
                  ))}
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <div><span className="text-xs text-slate-500">{stockT("fields.totalBuyingValue")}</span><p className="font-semibold">{money(receipt.total_buying_value)}</p></div>
                <div><span className="text-xs text-slate-500">{stockT("fields.stockExpenses")}</span><p className="font-semibold">{money(receipt.additional_cost)}</p></div>
              </div>
              <StockSummaryActions receipt={receipt} />
              <div className="flex flex-wrap gap-2">
                <Button disabled={!receipt.correction_open} size="small" type="button" variant="outline" onClick={() => {
                ...
                {receipt.status === "received" ? (
                  <Button size="small" type="button" variant="outline" onClick={() => resetDraft(receipt)}>{t("actions.addLateDelivery")}</Button>
                ) : (
                  <Button size="small" type="button" variant="ghost" onClick={() => void archiveDraft(receipt)}><Archive className="size-4" /> {t("actions.archiveDraft")}</Button>
                )}
              </div>
              ...
              {correctionId === receipt.id && correction ? (
                <div className="grid gap-3 border-t border-slate-200 pt-3 dark:border-slate-800">
                  <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    <label className={field}>{stockT("fields.stockExpenses")}<Input min="0" step="0.01" type="number" value={correction.stockExpenses} onChange={(event) => setCorrection({ ...correction, stockExpenses: event.target.value })} /></label>
                  </div>
                  {correction.batches.map((batch) => (
                    <div className="grid gap-3 rounded-lg border border-slate-200 p-3 dark:border-slate-800" key={batch.id}>
                      <label className={field}>{stockT("fields.batchName")}<Input value={batch.name} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((item) => item.id === batch.id ? { ...item, name: event.target.value } : item) })} /></label>
                      {batch.groups.map((group) => (
                        <div className="grid gap-3 rounded-lg bg-slate-50 p-3 dark:bg-slate-900" key={group.id}>
                          <div className="grid gap-2 sm:grid-cols-2">
                            <label className={field}>{stockT("fields.groupName")}<Input value={group.name} onChange={(event) => updateCorrectionGroup(batch.id, group.id, (current) => ({ ...current, name: event.target.value }))} /></label>
                            <label className={field}>{stockT("fields.unit")}<Select value={group.unit} onChange={(event) => updateCorrectionGroup(batch.id, group.id, (current) => ({ ...current, unit: event.target.value }))}>{unitOptions.map((option) => <option key={option} value={option}>{t(`units.${option}`)}</option>)}</Select></label>
                          </div>
                          {group.products.map((product) => (
                            <div className="grid gap-2 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950" key={product.id}>
                              <div className="grid gap-2 sm:grid-cols-2">
                                <label className={field}>{stockT("fields.productName")}<Input value={product.name} onChange={(event) => updateCorrectionProduct(batch.id, group.id, product.id, (current) => ({ ...current, name: event.target.value }))} /></label>
                                <label className={field}>{stockT("fields.productQuantity")}<Input min="0.001" step="0.001" type="number" value={product.quantity} onChange={(event) => updateCorrectionProduct(batch.id, group.id, product.id, (current) => ({ ...current, quantity: event.target.value }))} /></label>
                                <label className={field}>{t("fields.brandOptional")}<Input value={product.brand} onChange={(event) => updateCorrectionProduct(batch.id, group.id, product.id, (current) => ({ ...current, brand: event.target.value }))} /></label>
                                <label className={field}>{t("fields.variant")}<Input value={product.variant} onChange={(event) => updateCorrectionProduct(batch.id, group.id, product.id, (current) => ({ ...current, variant: event.target.value }))} /></label>
                                <label className={field}>{t("fields.barcode")}<Input value={product.barcode} onChange={(event) => updateCorrectionProduct(batch.id, group.id, product.id, (current) => ({ ...current, barcode: event.target.value }))} /></label>
                                <CostEditor mode={product.costMode} quantity={product.quantity} value={product.costValue} onMode={(mode) => updateCorrectionProduct(batch.id, group.id, product.id, (current) => ({ ...current, costMode: mode }))} onValue={(value) => updateCorrectionProduct(batch.id, group.id, product.id, (current) => ({ ...current, costValue: value }))} />
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          {receipts.length > 5 ? (
            <Button type="button" variant="ghost" onClick={() => setShowAll((value) => !value)}>{showAll ? t("actions.showLess") : t("actions.viewMore")}</Button>
          ) : null}
        </div>
      </aside>
    </section>
