...
type DraftBatch = { id: string; name: string; groups: DraftGroup[] };
...
type CorrectionBatch = { id: string; name: string; groups: CorrectionGroup[] };
...
function StockRecorder({ businessId }: { businessId: string }) {
  ...
  const newTrackedUnit = (): DraftTrackedUnit => ({
    id: nextId("unit"),
    modelName: "",
    brand: "",
    color: "",
  ...
  const newBatch = (name: string): DraftBatch => ({
    id: nextId("batch"),
    name,
    groups: [newGroup()],
  });
  ...
  const updateBatch = (batchId: string, change: (batch: DraftBatch) => DraftBatch) =>
    setBatches((current) =>
      current.map((batch) => (batch.id === batchId ? change(batch) : batch)),
    );
  const updateGroup = (
    batchId: string,
    groupId: string,
  ...
  const updateProduct = (
    ...
    groupId: string,
    productId: string,
    change: (product: DraftProduct) => DraftProduct,
  ) =>
  ...
  const chooseExistingProduct = (
    ...
    if (selected && selected.unit.casefold() !== groupUnit.casefold()) {
      notify({ message: stockT("validation.productUnitMismatch"), tone: "error" });
      return;
    }
    updateProduct(batchId, group.id, product.id, (item) => ({
      ...item,
  ...
  const validate = () => {
    for (const batch of batches) {
      ...
      if (!batch.groups.length) return stockT("validation.groupRequired");
      for (const group of batch.groups) {
        ...
        const productTotal = group.products.reduce(
          (total, product) => total + Number(product.quantity || 0),
          0,
        );
        if (productTotal !== groupQuantity) return stockT("validation.groupBalance");
        for (const product of group.products) {
          const qty = Number(product.quantity);
          if (!product.existingProductId && !product.name.trim())
            return t("validation.itemNameRequired");
          if (!Number.isFinite(qty) || qty <= 0) return t("validation.quantityPositive");
          if (countableUnits.has(unit) && !Number.isInteger(qty))
            return t("validation.wholeQuantity", { unit });
          if (product.costValue && normalizedUnitCost(product.costMode, product.costValue, product.quantity) == null)
            return stockT("validation.buyingCost");
          if (product.trackingMode === "individual") {
            if (!Number.isInteger(qty)) return stockT("validation.individualWhole");
            if (product.trackedUnits.length !== qty)
              return stockT("validation.individualCount", { count: qty });
          }
        }
      }
    }
  ...
  const submitStock = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      notify({ message: error, tone: "error" });
      return;
    }

    const catalogItems = batches.flatMap((batch) =>
            ...
            types: group.products.map((product) => ({
              ...
              tracked_units: product.trackedUnits.map((tracked) => ({
                ...
                identifiers: tracked.identifierValue.trim()
                  ? [
                      {
                        kind: tracked.identifierKind,
                        value: tracked.identifierValue.trim(),
                      },
                    ]
                  : [],
              })),
    ...
    setBusy(true);
    try {
      const response = await createStockReceipt(businessId, accessToken, body);
      const receipt = response.data as StockReceipt | null;
      if (receipt) setSavedReceipt(receipt);
      notify({ message: t("success.stock"), tone: "success" });
      await load();
    } catch (reason) {
      notify({
  ...
  const resetRecorder = (parent?: StockReceipt) => {
    ...
    setStockExpenses("0");
    setBatches([newBatch(parent?.batches[0]?.name || "Batch 1")]);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const startCorrection = (receipt: StockReceipt) => {
  ...
  const saveCorrection = async (receipt: StockReceipt) => {
    ...
    const lines = correction.batches.flatMap((batch) =>
      batch.groups.flatMap((group) =>
        group.products.map((product) => ({
          id: product.id,
          name: product.name.trim(),
          ...
          unit_cost: product.costValue
            ? normalizedUnitCost(product.costMode, product.costValue, product.quantity)
            : null,
        })),
      ),
    );
    ...
    try {
      await correctStockReceipt(businessId, receipt.id, accessToken, {
        ...
        batches: correction.batches.map((batch) => ({ id: batch.id, name: batch.name })),
        groups: correction.batches.flatMap((batch) =>
          batch.groups.map((group) => ({ id: group.id, name: group.name })),
        ),
        lines,
      });
    ...
    return (
      <section className={`${panel} grid gap-5`}>
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-950">
          <strong>{t("savedReceipt.received")}</strong>
            ...
            {batch.groups.map((group) => (
              <section className="grid gap-3 rounded-xl bg-slate-50 p-3 dark:bg-slate-900" key={group.id}>
                <div className="flex flex-wrap justify-between gap-2">
                  ...
                  <span className="text-sm">{quantityText(group.quantity)} {group.unit}</span>
                </div>
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-900">
            ...
            <p className="font-semibold">{amount(savedReceipt.total_buying_value)}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-900">
            <span className="text-slate-500">{stockT("fields.stockExpenses")}</span>
            <p className="font-semibold">{amount(savedReceipt.additional_cost)}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-900">
            <span className="text-slate-500">{stockT("summary.totalStockCost")}</span>
            <p className="font-semibold">
              {amount(String(Number(savedReceipt.total_buying_value) + Number(savedReceipt.additional_cost)))}
            </p>
          </div>
        </div>
  ...
  return (
    <section className="grid items-start gap-5 xl:grid-cols-[1.2fr_.8fr]">
      <form className={`${panel} grid gap-5`} onSubmit={submitStock}>
        <div>
        ...
        {batches.map((batch, batchIndex) => (
          <article className="grid gap-4 rounded-xl border border-slate-200 p-4 shadow-sm dark:border-slate-800" key={batch.id}>
            <div className="flex flex-wrap items-end gap-3">
              ...
              {batches.length > 1 ? (
                <Button type="button" variant="ghost" onClick={() => setBatches((current) => current.filter((item) => item.id !== batch.id))}>
            ...
            {batch.groups.map((group) => {
              const unit = group.unit === "other" ? group.customUnit : group.unit;
              ...
              return (
                <section className="grid gap-4 rounded-xl bg-slate-50 p-4 dark:bg-slate-900" key={group.id}>
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <label className={field}>
                      ...
                      <Select value={group.unit} onChange={(event) => updateGroup(batch.id, group.id, (current) => ({ ...current, unit: event.target.value, customUnit: event.target.value === "other" ? current.customUnit : "" }))}>
                        {unitOptions.map((option) => <option key={option} value={option}>{t(`units.${option}`)}</option>)}
                        {units.map((option) => <option key={option.id} value={option.name}>{option.name}</option>)}
                  ...
                  <p className="text-xs text-slate-500">
                    {stockT("groupBalance", { recorded: allocated, total: Number(group.quantity || 0), unit: unit || "—" })}
                  </p>
                    ...
                    <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950" key={product.id}>
                      <div className="grid gap-3 sm:grid-cols-2">
                        <label className={field}>
                          ...
                          <Select value={product.existingProductId} onChange={(event) => chooseExistingProduct(batch.id, group, product, event.target.value)}>
                            <option value="">{stockT("values.newProduct")}</option>
                            {products.filter((item) => item.is_active).map((item) => (
                              <option key={item.id} value={item.id}>{item.name} · {item.sku} · {item.unit}</option>
                            ))}
                          </Select>
                        </label>
                        {!product.existingProductId ? (
                        ...
                        <label className={field}>
                          {stockT("costMode.label")}
                          <div className="grid grid-cols-[9rem_1fr] gap-2">
                            <Select value={product.costMode} onChange={(event) => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, costMode: event.target.value as StockCostMode }))}>
                              <option value="per_unit">{stockT("costMode.perUnit")}</option>
                              <option value="total">{stockT("costMode.total")}</option>
                            </Select>
                            <Input min="0" step="0.01" type="number" value={product.costValue} onChange={(event) => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, costValue: event.target.value }))} />
                          </div>
                        </label>
                        <label className={field}>
                          {t("fields.tracking")}
                      ...
                      {product.trackingMode === "individual" ? (
                        <div className="grid gap-3 rounded-lg border border-slate-200 p-3 dark:border-slate-800">
                          <p className="text-xs font-semibold text-slate-600 dark:text-slate-300">
                            {stockT("individualProgress", { recorded: product.trackedUnits.length, total: Number(product.quantity || 0) })}
                          </p>
                          {product.trackedUnits.map((tracked, index) => (
                            <div className="grid gap-2 rounded-lg bg-slate-50 p-3 sm:grid-cols-2 dark:bg-slate-900" key={tracked.id}>
                              ...
                              <Select value={tracked.identifierKind} onChange={(event) => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, trackedUnits: current.trackedUnits.map((item) => item.id === tracked.id ? { ...item, identifierKind: event.target.value as DraftTrackedUnit["identifierKind"] } : item) }))}>
                                {identifierKinds.map((kind) => <option key={kind} value={kind}>{t(`identifierTypes.${kind}`)}</option>)}
                              </Select>
                          ...
                          <Button disabled={product.trackedUnits.length >= Math.floor(Number(product.quantity) || 0)} type="button" variant="outline" onClick={() => updateProduct(batch.id, group.id, product.id, (current) => ({ ...current, trackedUnits: [...current.trackedUnits, newTrackedUnit()] }))}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" onClick={() => updateGroup(batch.id, group.id, (current) => ({ ...current, products: [...current.products, newProduct()] }))}>
            ...
            <Button type="button" variant="outline" onClick={() => updateBatch(batch.id, (current) => ({ ...current, groups: [...current.groups, newGroup()] }))}>
              <Plus className="size-4" /> {t("actions.addGroup")}
            </Button>
          </article>
        ))}

        <Button type="button" variant="outline" onClick={() => setBatches((current) => [...current, newBatch(`Batch ${current.length + 1}`)])}>
      ...
      <aside className={`${panel} self-start`}>
        ...
        <div className="mt-4 grid gap-3 xl:max-h-[calc(100vh-12rem)] xl:overflow-y-auto xl:pr-2">
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-slate-500">{receipt.supplier_name || t("fields.supplierNotEntered")}</p>
                </div>
                  ...
                  {batch.groups.map((group) => (
                    <div className="grid gap-2 rounded-lg bg-slate-50 p-2 dark:bg-slate-900" key={group.id}>
                      <div className="flex justify-between gap-2">
                        <strong>{group.name}</strong>
                        <span>{quantityText(group.quantity)} {group.unit}</span>
                      </div>
                      {group.types.map((line) => (
                        <div className="flex justify-between gap-3 text-xs" key={line.id}>
                          ...
                          <span>{amount(line.received_unit_cost)} / {line.received_unit}</span>
                        </div>
                      ))}
                    </div>
                  ))}
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <div>
                  ...
                  <p className="font-semibold">{amount(receipt.total_buying_value)}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-500">{stockT("fields.stockExpenses")}</span>
                  <p className="font-semibold">{amount(receipt.additional_cost)}</p>
                </div>
              </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (correctionId === receipt.id) {
                      setCorrectionId("");
                      setCorrection(null);
                    } else startCorrection(receipt);
                  }}
                >
                  {t("actions.correctStock")}
                  {correctionId === receipt.id ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
                </Button>
                {receipt.status === "received" ? (
                  <Button size="small" type="button" variant="outline" onClick={() => resetRecorder(receipt)}>
              ...
              {correctionId === receipt.id && correction ? (
                <div className="grid gap-3 border-t border-slate-200 pt-3 dark:border-slate-800">
                      ...
                      {batch.groups.map((group) => (
                        <div className="grid gap-3 rounded-lg bg-slate-50 p-3 dark:bg-slate-900" key={group.id}>
                          <div className="grid gap-2 sm:grid-cols-2">
                            <label className={field}>
                              {stockT("fields.groupName")}
                              <Input value={group.name} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, name: event.target.value } : groupItem) } : batchItem) })} />
                            </label>
                            <label className={field}>
                              {stockT("fields.unit")}
                              <Select value={group.unit} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, unit: event.target.value, products: groupItem.products.map((product) => ({ ...product, unit: event.target.value })) } : groupItem) } : batchItem) })}>
                                {unitOptions.map((option) => <option key={option} value={option}>{t(`units.${option}`)}</option>)}
                              </Select>
                            </label>
                          </div>
                          {group.products.map((product) => (
                            <div className="grid gap-2 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950" key={product.id}>
                              <div className="grid gap-2 sm:grid-cols-2">
                                <label className={field}>{stockT("fields.productName")}<Input value={product.name} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, products: groupItem.products.map((item) => item.id === product.id ? { ...item, name: event.target.value } : item) } : groupItem) } : batchItem) })} /></label>
                                <label className={field}>{stockT("fields.productQuantity")}<Input min="0.001" step="0.001" type="number" value={product.quantity} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, products: groupItem.products.map((item) => item.id === product.id ? { ...item, quantity: event.target.value } : item) } : groupItem) } : batchItem) })} /></label>
                                <label className={field}>{t("fields.brandOptional")}<Input value={product.brand} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, products: groupItem.products.map((item) => item.id === product.id ? { ...item, brand: event.target.value } : item) } : groupItem) } : batchItem) })} /></label>
                                <label className={field}>{t("fields.variant")}<Input value={product.variant} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, products: groupItem.products.map((item) => item.id === product.id ? { ...item, variant: event.target.value } : item) } : groupItem) } : batchItem) })} /></label>
                                <label className={field}>{t("fields.barcode")}<Input value={product.barcode} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, products: groupItem.products.map((item) => item.id === product.id ? { ...item, barcode: event.target.value } : item) } : groupItem) } : batchItem) })} /></label>
                                <label className={field}>
                                  {stockT("costMode.label")}
                                  <div className="grid grid-cols-[8rem_1fr] gap-2">
                                    <Select value={product.costMode} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, products: groupItem.products.map((item) => item.id === product.id ? { ...item, costMode: event.target.value as StockCostMode } : item) } : groupItem) } : batchItem) })}>
                                      <option value="per_unit">{stockT("costMode.perUnit")}</option>
                                      <option value="total">{stockT("costMode.total")}</option>
                                    </Select>
                                    <Input min="0" step="0.01" type="number" value={product.costValue} onChange={(event) => setCorrection({ ...correction, batches: correction.batches.map((batchItem) => batchItem.id === batch.id ? { ...batchItem, groups: batchItem.groups.map((groupItem) => groupItem.id === group.id ? { ...groupItem, products: groupItem.products.map((item) => item.id === product.id ? { ...item, costValue: event.target.value } : item) } : groupItem) } : batchItem) })} />
                                  </div>
                                </label>
                              </div>
                            </div>
                  ...
                  <div className="flex gap-2">
                    <Button disabled={busy} size="small" type="button" onClick={() => void saveCorrection(receipt)}>{t("actions.saveCorrection")}</Button>
                    <Button size="small" type="button" variant="ghost" onClick={() => { setCorrectionId(""); setCorrection(null); }}>{t("actions.cancel")}</Button>
                  </div>
                </div>
              ) : null}
                ...
                <div className="grid gap-2 border-l-2 border-orange-400 pl-3">
                  ...
                  {receipt.late_deliveries.map((delivery) => (
                    <div className="rounded-lg bg-slate-50 p-2 dark:bg-slate-900" key={delivery.id}>
                      <div className="flex justify-between gap-2"><span>{delivery.reference}</span><span>{delivery.received_at ? new Date(delivery.received_at).toLocaleDateString() : "—"}</span></div>
                      <p className="text-xs text-slate-500">{delivery.quantities_by_unit.map((item) => `${item.quantity} ${item.unit}`).join(" · ")}</p>
                    </div>
                  ))}
                </div>
              ) : null}
          ...
          {!receipts.length ? <p className="text-sm text-slate-500">{t("empty.stock")}</p> : null}
          {receipts.length > 5 ? (
            <Button type="button" variant="ghost" onClick={() => setShowAll((current) => !current)}>
