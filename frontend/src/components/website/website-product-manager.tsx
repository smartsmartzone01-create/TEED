    ...
    <div className="md:col-span-6">
      <div className="mb-2 flex items-baseline justify-between gap-3">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
          {sw ? "Picha za aina" : "Variant gallery"}
        </p>
      ...
      {media.length ? (
        <div className="flex flex-wrap gap-2">
          {media.map((item) => {
            ...
            return (
              <button
                className={`relative overflow-hidden rounded-lg border p-1 text-left ${selected ? "border-slate-950 ring-1 ring-slate-950 dark:border-white dark:ring-white" : "border-slate-200 dark:border-slate-800"}`}
                disabled={!canManage}
                key={item.id}
                ...
                type="button"
              >
                <img alt="" className="h-16 w-16 rounded-md object-cover" src={item.public_url} />
                {selected ? (
        ...
        <p className="text-xs text-slate-400">{sw ? "Pakia picha kwenye Media kwanza." : "Upload images in Media first."}</p>
      )}
    </div>
  );
}

function VariantEditor({ businessId, canManage, listingId, media, onChanged, siteId, sw, variant }: {
  businessId: string;
  canManage: boolean;
  listingId: string;
  media: WebsiteMedia[];
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await request((token) => updateWebsiteVariant(
        businessId,
        siteId,
        listingId,
        variant.id,
      ...
      notify({ message: error instanceof Error ? error.message : sw ? "Imeshindikana kuhifadhi aina." : "Variant could not be saved.", tone: "error" });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function remove() {
    ...
    try {
      ...
      notify({ message: error instanceof Error ? error.message : sw ? "Imeshindikana kufuta aina." : "Variant could not be deleted.", tone: "error" });
    }
  }
    ...
    <div className="grid gap-3 rounded-lg border border-slate-200 p-3 dark:border-slate-800 md:grid-cols-6">
      <div className="md:col-span-2">
        ...
        <p className="mt-1 text-xs text-slate-500">
          {Object.entries(variant.options).map(([key, value]) => `${key}: ${value}`).join(" · ") || (sw ? "Hakuna chaguo" : "No options")}
        </p>
        {variant.commerce_product_id ? <p className="mt-1 text-[11px] text-slate-400">Commerce linked</p> : null}
      </div>
      <input aria-label={sw ? "Bei" : "Price"} className={fieldClassName} disabled={!canManage} inputMode="decimal" onChange={(event) => setPrice(event.target.value)} placeholder="0.00" value={price} />
      ...
      <div className="flex items-center gap-2 md:col-span-2 md:justify-end">
        <label className="flex items-center gap-2 text-xs">
          <input checked={published} disabled={!canManage} onChange={(event) => setPublished(event.target.checked)} type="checkbox" />
          {sw ? "Imechapishwa" : "Published"}
        </label>
        {canManage ? (
          <>
            <button aria-label={sw ? "Hifadhi aina" : "Save variant"} className="inline-flex size-8 items-center justify-center rounded-md border border-slate-200 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900" disabled={saving} onClick={() => void save()} type="button">
              {saving ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}
            </button>
            <button aria-label={sw ? "Futa aina" : "Delete variant"} className="inline-flex size-8 items-center justify-center rounded-md text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30" onClick={() => void remove()} type="button"><Trash2 className="size-4" /></button>
          </>
        ) : null}
      </div>
      <MediaPicker canManage={canManage} media={media} selectedIds={galleryMediaIds} setSelectedIds={setGalleryMediaIds} sw={sw} />
    </div>
  );
}
...
function WebsiteProductManager({ businessId }: { businessId: string }) {
  ...
  const copy = useMemo(() => ({
    ...
    subtitle: sw
      ? "Chagua picha kuu ya orodha, kisha unganisha picha za kila aina kamili."
      : "Choose one listing cover, then attach an ordered gallery to each exact variant.",
  }), [sw]);
  ...
  function draftFor(listingId: string): VariantDraft {
    return variantDrafts[listingId] ?? { availability: "in_stock", galleryMediaIds: [], options: "{}", price: "", sku: "" };
  }
  ...
  return (
    <div className="space-y-5">
      <header><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Website</p><h1 className="mt-1 text-2xl font-semibold text-slate-950 dark:text-white">{copy.title}</h1><p className="mt-1 text-sm text-slate-500">{copy.subtitle}</p></header>
      {canManage ? (
        <section className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            ...
            <select className={fieldClassName} onChange={(event) => setPrimaryMediaId(event.target.value)} value={primaryMediaId}><option value="">{sw ? "Hakuna picha kuu" : "No listing cover"}</option>{media.map((item) => <option key={item.id} value={item.id}>{item.original_name || item.id}</option>)}</select>
          </div>
          ...
          return (
            <article className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5" key={listing.id}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div><h2 className="font-semibold">{listing.title[locale] || listing.title.en || listing.slug}</h2><p className="mt-1 text-xs text-slate-500">/{listing.slug}{listing.brand ? ` · ${listing.brand}` : ""}</p></div>
              ...
              <div className="mt-4 space-y-2">{listing.variants.map((variant) => <VariantEditor businessId={businessId} canManage={canManage} key={variant.id} listingId={listing.id} media={media} onChanged={load} siteId={site.id} sw={sw} variant={variant} />)}</div>
              {canManage ? (
                <div className="mt-4 rounded-lg bg-slate-50 p-3 dark:bg-slate-900/50">
                  ...
                  <div className="mt-3"><MediaPicker canManage media={media} selectedIds={draft.galleryMediaIds} setSelectedIds={(ids) => updateDraft(listing.id, { galleryMediaIds: ids })} sw={sw} /></div>
                  <Button className="mt-3" onClick={() => void addVariant(listing)} size="small"><Plus className="size-4" />{sw ? "Ongeza aina" : "Add variant"}</Button>
                </div>
              ) : null}
            </article>
          );
