...
function MediaThumbnail({ item, onRemove, order }: { item: WebsiteMedia; onRemove?: () => void; order?: number }) {
  return (
    <div className="relative size-16 overflow-hidden rounded-lg border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900">
      ...
      />
      {order ? <span className="absolute left-1 top-1 rounded-full bg-slate-950 px-1.5 py-0.5 text-[10px] font-bold text-white dark:bg-white dark:text-slate-950">{order}</span> : null}
      {onRemove ? (
        <button aria-label="Remove image" className="absolute right-1 top-1 inline-flex size-5 items-center justify-center rounded-full bg-white/95 text-slate-700 shadow-sm hover:bg-white" onClick={onRemove} type="button">
          <X className="size-3" />
        </button>
      ) : null}
    </div>
...
function MediaPicker({ canManage, label, media, onUpload, selectedIds, setSelectedIds, single = false, sw }: {
  ...
  const selectedMedia = selectedIds.map((id) => media.find((item) => item.id === id)).filter((item): item is WebsiteMedia => Boolean(item));
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {canManage ? (
          <button className="inline-flex h-8 items-center gap-1.5 rounded-md border border-slate-200 px-2.5 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-900" onClick={() => setMenuOpen((current) => !current)} type="button">
      ...
      {selectedMedia.length ? (
        <div className="flex flex-wrap gap-2">
          {selectedMedia.map((item, index) => (
            <MediaThumbnail item={item} key={item.id} onRemove={canManage ? () => setSelectedIds(selectedIds.filter((id) => id !== item.id)) : undefined} order={single ? undefined : index + 1} />
          ))}
        </div>
      ...
      {menuOpen ? (
        <div className="flex flex-wrap gap-2 rounded-md border border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900/60">
          <label className={`inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs font-medium dark:border-slate-700 dark:bg-slate-950 ${uploading ? "pointer-events-none opacity-60" : ""}`}>
            ...
            <input accept="image/jpeg,image/png,image/webp" className="hidden" disabled={uploading} onChange={(event) => { const input = event.currentTarget; const file = input.files?.[0]; if (file) void upload(file).finally(() => { input.value = ""; }); }} type="file" />
          </label>
          <button className="inline-flex h-9 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs font-medium dark:border-slate-700 dark:bg-slate-950" onClick={() => { setGalleryOpen((current) => !current); setMenuOpen(false); }} type="button">
        ...
        <div className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
          <div className="mb-2 flex items-center justify-between gap-3">
            <div>
              ...
              {!single ? <p className="mt-0.5 text-[11px] text-slate-400">{sw ? "Mpangilio wa kuchagua ndio mpangilio wa thumbnails." : "Selection order becomes thumbnail order."}</p> : null}
            </div>
            <Button onClick={() => setGalleryOpen(false)} size="small" type="button" variant="ghost">{sw ? "Funga" : "Done"}</Button>
          </div>
          {media.length ? (
            <div className="grid max-h-64 grid-cols-3 gap-2 overflow-y-auto pr-1 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-9">
              {media.map((item) => {
                ...
                return (
                  <button className={`relative aspect-square overflow-hidden rounded-lg border p-1 ${selected ? "border-slate-950 ring-1 ring-slate-950 dark:border-white dark:ring-white" : "border-slate-200 dark:border-slate-800"}`} key={item.id} onClick={() => toggle(item.id)} title={item.original_name || item.id} type="button">
                    <span aria-label={item.original_name || "Website image"} className="block size-full rounded-md bg-cover bg-center" role="img" style={{ backgroundImage: `url("${item.public_url}")` }} />
                    {selected && !single ? <span className="absolute right-1 top-1 rounded-full bg-slate-950 px-1.5 py-0.5 text-[10px] font-bold text-white dark:bg-white dark:text-slate-950">{selectedIndex + 1}</span> : null}
                  </button>
                );
              })}
            </div>
...
type VariantDraft = { price: string; availability: WebsiteVariantAvailability; options: string; galleryMediaIds: string[] };
...
function WebsiteProductManager({ businessId }: { businessId: string }) {
  ...
  async function createListing() {
    ...
    setCreating(true);
    try {
      await request((token) => createWebsiteListing(businessId, site.id, {
        brand: brand.trim(), is_published: false, primary_media_id: primaryMediaId || null, slug: slug.trim(), title: { en: titleEn.trim(), sw: titleSw.trim() },
      }, token));
      setTitleEn(""); setTitleSw(""); setSlug(""); setBrand(""); setPrimaryMediaId("");
  ...
  function draftFor(listingId: string): VariantDraft {
    return variantDrafts[listingId] ?? { availability: "in_stock", galleryMediaIds: [], options: "{}", price: "" };
  }
  ...
  async function addWebsiteSku(listing: WebsiteListing) {
    ...
    try {
      await request((token) => createWebsiteVariant(businessId, site.id, listing.id, {
        currency: "TZS", gallery_media_ids: draft.galleryMediaIds, is_published: false, options, website_availability: draft.availability, website_price: draft.price.trim() || null,
      }, token));
      ...
      notify({ message: createError instanceof Error ? createError.message : sw ? "SKU haijaundwa." : "Website SKU could not be created.", tone: "error" });
    }
  }
    ...
    <div className="space-y-5">
      ...
      {canManage ? (
        <section className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
          ...
          <p className="mt-1 text-xs text-slate-500">{sw ? "Unda bidhaa binafsi au familia ya Website. SKU zinaongezwa ndani yake na kutengenezwa kiotomatiki." : "Create a standalone Website product or family. Its Website SKUs are added inside and generated automatically."}</p>
          ...
          <div className="mt-3 max-w-xl"><MediaPicker canManage label={sw ? "Picha ya kuonyesha bidhaa" : "Product showcase image"} media={media} onUpload={uploadMedia} selectedIds={primaryMediaId ? [primaryMediaId] : []} setSelectedIds={(ids) => setPrimaryMediaId(ids[0] ?? "")} single sw={sw} /></div>
            ...
            <article className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5" key={listing.id}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">/{listing.slug}{listing.brand ? ` · ${listing.brand}` : ""}{listing.variants.length ? ` · ${listing.variants.length} SKU` : ""}</p>
                </div>
                  ...
                  {skuFormOpen ? (
                    <div className="mt-3 rounded-lg bg-slate-50 p-3 dark:bg-slate-900/50">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{sw ? "SKU mpya ya Website" : "New Website SKU"}</p><p className="mt-1 text-[11px] text-slate-400">{sw ? "Nambari ya SKU itatengenezwa kiotomatiki na Website. SKU mpya inaanza kama rasimu." : "The Website generates the SKU automatically. New Website SKUs start as drafts."}</p></div>
                      ...
                      <div className="mt-3"><MediaPicker canManage label={sw ? "Picha za SKU" : "SKU images"} media={media} onUpload={uploadMedia} selectedIds={draft.galleryMediaIds} setSelectedIds={(ids) => updateDraft(listing.id, { galleryMediaIds: ids })} sw={sw} /></div>
